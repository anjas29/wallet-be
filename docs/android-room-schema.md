...
data class LiabilityEntity(
    ...
    val paidAmount: String?,       // derived, server-computed — added v1.2.0, not sent on push
    val remainingBalance: String?, // derived, server-computed — added v1.2.0, not sent on push
    val createdAt: String?,
    val updatedAt: String,
    val deletedAt: String?,
    val pendingOp: PendingOp = PendingOp.NONE,
...
// Added v1.2.0. `spent`/`remaining` are derived server-side (never stored/pushed) — refresh them
// from the latest pull/read rather than computing locally.
@Entity(
    tableName = "budgets",
    indices = [Index("userId"), Index("categoryId"), Index("updatedAt"), Index("pendingOp")],
)
data class BudgetEntity(
    @PrimaryKey val id: String,
    val userId: String,
    val categoryId: String,        // → user_categories.id, must be an expense category
    val amount: String,            // the budget cap
    val periodType: String,        // monthly | custom
    val periodStart: String?,      // set only when periodType = custom
    val periodEnd: String?,        // set only when periodType = custom
    val spent: String?,            // derived, server-computed — not sent on push
    val remaining: String?,        // derived, server-computed — not sent on push
    val createdAt: String?,
    val updatedAt: String,
    val deletedAt: String?,
    val pendingOp: PendingOp = PendingOp.NONE,
...
// Added v1.2.0. A template that generates ordinary TransactionEntity rows server-side on a
// schedule — there is no isAutoGenerated flag on TransactionEntity, and none is planned; a
// generated transaction is fully editable/deletable like any other row.
@Entity(
    tableName = "recurring_transactions",
    indices = [Index("userId"), Index("accountId"), Index("categoryId"), Index("updatedAt"), Index("pendingOp")],
)
data class RecurringTransactionEntity(
    ...
    val endDate: String?,          // nullable stop condition
    val nextRunDate: String?,      // server-computed, read-only — client sends are ignored
...
```kotlin
@Database(
    version = 3,   // bumped 2 → 3 in v1.2.0: added BudgetEntity + RecurringTransactionEntity
    entities = [
...
**Migration 2 → 3** (v1.2.0):

```kotlin
val MIGRATION_2_3 = object : Migration(2, 3) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL("ALTER TABLE liabilities ADD COLUMN paidAmount TEXT")
        db.execSQL("ALTER TABLE liabilities ADD COLUMN remainingBalance TEXT")
        ...
        db.execSQL("CREATE INDEX IF NOT EXISTS index_budgets_updatedAt ON budgets(updatedAt)")
        db.execSQL("CREATE INDEX IF NOT EXISTS index_budgets_pendingOp ON budgets(pendingOp)")
        ...
        db.execSQL("CREATE INDEX IF NOT EXISTS index_recurring_transactions_updatedAt ON recurring_transactions(updatedAt)")
        db.execSQL("CREATE INDEX IF NOT EXISTS index_recurring_transactions_pendingOp ON recurring_transactions(pendingOp)")
    }
}
```
...
- **Order of applying a pull:** reference (currencies, categories) → user_currencies → user_categories → accounts → transactions/transfers → liabilities → liability_payments → budgets → recurring_transactions, so relations resolve.
- **Derived, never stored:** account balance, transaction currency, liability `paidAmount`/`remainingBalance` (v1.2.0), budget `spent`/`remaining`.
- **Server-generated transactions:** `recurring_transaction` templates produce ordinary rows in the existing `transactions` pull/push channel — no separate handling needed, and no marker distinguishes them from manually entered transactions.

---

## Changelog

- **1.2.0** (2026-08-24) — Room `@Database` `version = 2` → `3` (`MIGRATION_2_3`).
  - Added `BudgetEntity` (`budgets`, pushed + pulled). `spent`/`remaining` are server-derived, never sent on push.
  - Added `RecurringTransactionEntity` (`recurring_transactions`, pushed + pulled). `nextRunDate` is server-computed and read-only.
  - Added `LiabilityEntity.paidAmount`/`.remainingBalance` (both derived, server-computed, not sent on push).
